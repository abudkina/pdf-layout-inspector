...
import {
  ...
  проверитьUrl,
  проверитьИзображение,
  ограничить,
  прозрачностьИзПроцентов,
  процентыИзПрозрачности,
} from '../../src/utils/validation';
import { МАКС_РАЗМЕР_ФАЙЛА, МАКС_РАЗМЕР_ИЗОБРАЖЕНИЯ } from '../../src/types';
...
describe('проверка изображения', () => {
  it('принимает PNG', () => {
    const файл = new File(['x'], 'макет.png', { type: 'image/png' });
    expect(проверитьИзображение(файл).ок).toBe(true);
  });

  it('отклоняет PDF как картинку', () => {
    const файл = new File(['x'], 'док.pdf', { type: 'application/pdf' });
    const р = проверитьИзображение(файл);
    expect(р.ок).toBe(false);
    expect(р.ошибка).toMatch(/формат|PNG|JPEG/i);
  });

  it('отклоняет слишком большое изображение', () => {
    const файл = new File([new Uint8Array(10)], 'huge.png', {
      type: 'image/png',
    });
    Object.defineProperty(файл, 'size', { value: МАКС_РАЗМЕР_ИЗОБРАЖЕНИЯ + 1 });
    expect(проверитьИзображение(файл).ок).toBe(false);
  });
});
