...
export class Приложение {
  ...
  private async инициализироватьДокумент(
    ...
    // Сразу показываем UI, чтобы пользователь не смотрел на пустой экран
    this.разметка = {
      имяФайла,
      числоСтраниц: документ.numPages,
      страницы: Array.from({ length: документ.numPages }, (_, i) => ({
        номер: i + 1,
        ширина: 0,
        высота: 0,
        тексты: [],
    ...
    // 1) Сначала отрисовка страницы на canvas
    await this.показатьСтраницу(1);

    // 2) Затем разбор текстовых блоков / изображений
    ...
    try {
      ...
      const текущая = страницы.find(
        (с) => с.номер === this.настройки.номерСтраницы,
      );
      if (текущая) {
        this.просмотрщик.отрисоватьРазметку(текущая);
        this.обновитьСтатистику(
          текущая.тексты.length,
          текущая.изображения.length,
        );
      }
      ...
      // Страница уже на экране — показываем ошибку анализа, не сбрасываем документ
      this.обработатьОшибку(ошибка);
    } finally {
      this.скрытьЗагрузку();
    }
  ...
  private async показатьСтраницу(номер: number): Promise<void> {
    ...
    try {
      ...
      // Если разметка ещё пустая (ширина 0) — только canvas, оверлеи позже
      if (страница && страница.ширина > 0) {
        ...
        страница.ширина = результат.ширина;
        страница.высота = результат.высота;
      }
